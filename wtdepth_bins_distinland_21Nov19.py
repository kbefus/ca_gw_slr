                        ...
                        with memfile.open(**profile) as dataset:
                            ...
                            for temp_type in unique_types:
                                ...
                                for ifeature in temp_shp.geometry.values:
                                    ...
                                    mask_dist[mask_dist==other_nan_val]=np.nan
                                ...
                                if len(type_all)>0:
                                    out_stats.append([county_name,sl,Kh,d2,temp_type,
                                                      np.nanmedian(type_all),np.nanmean(type_all),
                                                      np.nanmax(type_all),np.nanmin(type_all),
                                                      np.nanstd(type_all)])
...
out_fname2 = os.path.join(results_dir,'wtdepth_bins_distinland_stats_{}.csv'.format(active_date))
out_cols2 = ['County','Sea_level_m','Kh_mday','Datum_Model',
             'WT_bin','Median_dist_m','Mean_dist_m','Max_dist_m',
             'Min_dist_m','Std_dist_m']
