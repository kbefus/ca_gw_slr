...
if __name__ == "__main__":
    ...
    parser.add_argument("-m","--mtype",help="Model type, 1=MHHW, 2=LMSL, not used=both",type=int)
    parser.add_argument("-k","--kvals",type=float)
    parser.add_argument("-s","--sealevel",type=float)
    parser.add_argument("-l","--linear",action="store_true",help="Run for linear responses")
    ...
    if args.kvals is None:
        Kh_vals = Kh_vals
    else:
        if isinstance(args.kvals,(list,np.ndarray)):
            Kh_vals = args.kvals
        else:
            Kh_vals = [args.kvals]
    
    if args.sealevel is None:
        sealevel_elevs = sealevel_elevs
    else:
        if isinstance(args.sealevel,(list,np.ndarray)):
            sealevel_elevs = args.sealevel
        else:
            sealevel_elevs = [args.sealevel]
